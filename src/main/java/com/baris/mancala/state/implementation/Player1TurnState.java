...
public class Player1TurnState extends AbstractGameState {
    @Override
    ...
    public IGameState play(Game game, int pit) throws IllegalMoveException, UnsupportedOperationException {
        validateMove(Side.PLAYER1, game, pit);
        int index = sow(game, PLAYER2_BIG_PIT, pit);
        if (index == PLAYER1_BIG_PIT) {
            ...
            if (board[index] == 1) {
                ...
                board[12 - index] = 0;
                board[index] = 0;
                board[PLAYER1_BIG_PIT] = board[PLAYER1_BIG_PIT] + stolenStones + 1;
                game.setBoard(board);
            }
